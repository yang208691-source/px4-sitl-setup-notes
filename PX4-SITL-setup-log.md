...
## 读之前：先分清两种 `VirtioProxy` 回退

如果你看到 WSL 启动时提示回退到 `VirtioProxy`，**有两种完全不同的原因，排查方向也完全不同**：

- **「检测到 localhost 代理配置，但未镜像到 WSL (networkingMode Nat)，因此回退到 VirtioProxy」**
  → **代理注入失败**。原因和排查见本文 **坑 2**
- **「无法配置网络 (networkingMode Nat)，回退到 VirtioProxy」**
  → **NAT 网络本身建不起来**。常见诱因：HNS 中残留了损坏的网络对象、已断开网卡的残留默认路由、**USB 网络共享上行不被 ICS 支持**

两者都表现为 `wslinfo --networking-mode` 报 `virtioproxy`，但一个是代理问题，一个是 Windows 网络栈问题。

**本文只覆盖第一种**（我实际遇到的）。第二种的排查方向是清理 HNS 对象和残留路由。
